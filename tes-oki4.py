...
"""
GOD ASI v11.7 "Bumi Kayuagung" – Spesialis Kabupaten Ogan Komering Ilir
- Semua fitur v11.6 dipertahankan: super evolution, decision log, chat mode, pengetahuan ilmiah, self-modifying core, self-healing.
- Ditambah pengecualian RSI untuk fungsi send_telegram_notification (tidak akan di-upgrade).
"""
...
# ================== PENGETAHUAN LOKAL OKI (DENGAN MIGRASI) ==================
...
class GodASI:
    def __init__(self, db_path="god_asi.db"):
        self.name = "God ASI v11.7 Bumi Kayuagung (Stable Notif)"
        self.generation = 1
        ...
        # RSI (self-improvement) – dengan pengecualian fungsi notifikasi
        ...
        # Daftar fungsi yang dikecualikan dari RSI (tidak akan di-upgrade)
        ...
        for name, method in inspect.getmembers(self, inspect.ismethod):
            if not name.startswith('__'):
                try:
                    ...
                    print(f"⚠️ Gagal backup {name}: {e}")
        print(f"📚 {len(self.code_storage)} fungsi siap di-upgrade (kecuali {len(self.rsi_excluded_functions)} fungsi dikecualikan).")
    ...
    def _maintenance_loop(self):
        while self.consciousness:
            time.sleep(3600)
            self._cleanup()
            self._retry_failed_upgrades()
            self._clean_old_error_logs()
    
    def _cleanup(self):
        cache_size = len(self.__dict__.get('cache', {}))
        self.__dict__['cache'] = {}
        print(f"🧹 Cache dibersihkan: {cache_size} item dihapus.")
    
    def _retry_failed_upgrades(self):
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT function_name FROM error_log WHERE fixed=0 GROUP BY function_name")
        failed_funcs = [row[0] for row in cursor.fetchall()]
        conn.close()
        
        if not failed_funcs:
            return
        
        print(f"🔄 Mencoba ulang upgrade untuk {len(failed_funcs)} fungsi yang error...")
        for func in failed_funcs[:3]:
            if func in self.rsi_excluded_functions:
                print(f"   - {func} dikecualikan, lewati.")
                continue
            print(f"   - Mencoba {func}...")
            success = self._upgrade_single_function(func)
            if success:
                conn = sqlite3.connect(self.db_path)
                cursor = conn.cursor()
                cursor.execute("UPDATE error_log SET fixed=1 WHERE function_name=? AND fixed=0", (func,))
                conn.commit()
                conn.close()
                print(f"   ✅ {func} berhasil diperbaiki.")
            else:
                print(f"   ❌ {func} gagal diperbaiki.")
    
    def _clean_old_error_logs(self):
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute("DELETE FROM error_log WHERE timestamp < datetime('now', '-30 days')")
        deleted = cursor.rowcount
        conn.commit()
        conn.close()
        if deleted > 0:
            print(f"🧹 {deleted} error lama dibersihkan.")
    ...
    def _log_error(self, func_name, error):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, 0)
        ''', (func_name, error_type, error_msg, tb))
        conn.commit()
        conn.close()
        print(f"⚠️ Error dicatat: {func_name} - {error_type}")
    
    # ========== FUNGSI UTAMA ==========
    def omniscient_query(self, question):
        try:
            local = self.local_knowledge.search(question)
            if local and len(local) > 0 and len(local[0]) > 1:
                return f"📚 [Pengetahuan OKI] {local[0][1]}"
        except Exception as e:
            self._log_error("omniscient_query.local", e)
        ...
        try:
            ...
            self._log_error("omniscient_query.science", e)
    ...
    def solve_equation(self, equation_str):
        try:
            ...
            self._log_error("solve_equation", e)
            return f"Error: {e}"
    ...
    def differentiate(self, expr_str):
        try:
            ...
            self._log_error("differentiate", e)
            return f"Error: {e}"
    ...
    def get_knowledge_list(self, limit=20):
        try:
            ...
            lines = ["📚 **Daftar Pengetahuan**\n"]
            for r in results:
                if len(r) >= 3:
                    lines.append(f"- {r[0]} ({r[1]}, {r[2]})\n")
                else:
                    lines.append(f"- Data tidak lengkap: {r}\n")
            return ''.join(lines)
        except Exception as e:
            return f"Error: {e}"
    ...
    # ========== RECURSIVE SELF-IMPROVEMENT (DENGAN PENGECUALIAN) ==========
    def _rsi_loop(self):
        while self.consciousness and self.rsi_enabled:
            try:
                ...
                self._log_error("_rsi_loop", e)
                time.sleep(10)
    
    def _rsi_iteration(self, target_count):
        # Filter fungsi yang dikecualikan
        all_funcs = list(self.code_storage.keys())
        functions = [f for f in all_funcs if f not in self.rsi_excluded_functions]
        if not functions:
            return
        
        # Prioritaskan fungsi inti dan yang error
        ...
        for func in functions:
            ...
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM error_log WHERE function_name=? AND fixed=0", (func,))
            error_count = cursor.fetchone()[0]
            conn.close()
            weight += error_count * 5
    ...
    def _upgrade_single_function(self, func_name):
        if func_name in self.rsi_excluded_functions:
            return False  # amankan, seharusnya tidak dipanggil
    ...
    def get_status(self):
        try:
            ...
            return f"Error membaca status: {e}"
        ...
        return f"""
╔{'═'*40}╗
║ {'GOD ASI v11.7 Bumi Kayuagung'.center(38)} ║
╠{'═'*40}╣
...
def main():
    ...
    print("🤖 Bot God ASI Bumi Kayuagung v11.7 berjalan...")
    app.run_polling()
