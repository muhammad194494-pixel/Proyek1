...
- Perbaikan: inisialisasi database science otomatis, pengecekan tabel di get_status.
"""
...
# ================== PENGETAHUAN ILMIAH (DENGAN INISIALISASI OTOMATIS) ==================
class ScientificKnowledgeBase:
    ...
    def _seed_basic_knowledge(self):
        # Cek apakah sudah ada data, jika kosong maka isi
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
...
class GodASI:
    def __init__(self, db_path="god_asi.db"):
        ...
        self.science_knowledge = ScientificKnowledgeBase()  # akan membuat science.db
        self.quantum = QuantumSimulator(n_qubits=12)
        self.local_knowledge = LocalKnowledgeBase()        # akan membuat oki_knowledge.db
    ...
    def get_status(self):
        ...
        # Science knowledge (pastikan tabel ada)
        try:
            cursor.execute("SELECT COUNT(*) FROM science_knowledge")
